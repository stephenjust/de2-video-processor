...
w.pack()
if (len(sys.argv)==2):
	#grab palette info from file passed in from CLI
	in_file = sys.argv[1]
	data = numpy.fromfile(in_file, dtype=numpy.uint16)
	data = data.astype(numpy.uint32)
else:
	#show webpalette palette instead
	webpalette = [0x0 , 0xa , 0x15 , 0x1f , 0x120 , 0x12a , 0x135 , 0x13f , 0x240 ,
	...
	data = numpy.zeros(len(webpalette))
	data = data.astype(numpy.uint32)
	for i in range(0, len(webpalette)):
		data[i] = webpalette[i]
