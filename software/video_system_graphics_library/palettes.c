 ...
 * palettes.c
 *
 * Definitions for default colour palettes and utility functions
 *
...
const palette_t palette_ega = {
		.colors = {
			/* 00 */ 0x0000, 0x0015, 0x2704, 0x1E79, /* Black    Blue       Green     Cyan    */
			/* 04 */ 0xA800, 0xA815, 0xE3C1, 0xAD55, /* Red      Magenta    Brown     Lt Grey */
			/* 08 */ 0x52AA, 0x52BF, 0x57EA, 0x57FF, /* Dk Grey  Lt Blue    B Green   B Cyan  */
			/* 12 */ 0xFAAA, 0xFABF, 0xFFEA, 0xFFFF  /* B Red    B Magenta  B Yellow  White   */
		}
};

const palette_t palette_332 = {
		.colors = {
...
const palette_t palette_magenta = {.colors = {0xA815}};

const palette_t palette_bunch_o_blues = {.colors = {0x000 , 0x001 , 0x002 , 0x003 , 0x004 , 0x005 , 0x006 , 0x007 , 0x008 ,
		0x009 , 0x00a , 0x00b , 0x00c , 0x00d , 0x00e , 0x00f , 0x0010 ,
		0x0011 , 0x0012 , 0x0013 , 0x0014 , 0x0015 , 0x0016 , 0x0017 , 0x0018 ,
		0x0019 , 0x001a , 0x001b , 0x001c , 0x001d , 0x001e , 0x001f }};

const palette_t palette_bunch_o_reds = {.colors = {0x000 , 0x010 , 0x020 , 0x030 , 0x040,
...
void switch_palette(palette_t *palette)
{
	...
	{
		IOWR_16DIRECT(COLOUR_PALETTE_SHIFTER_0_BASE, 2*i, palette->colors[i]);
	}
}
...
void print_palette(int n)
{
	...
	int i;
	palette_t current_palette;

	for (i = 0; i < n; i++){
		current_palette.colors[i] = IORD_16DIRECT(COLOUR_PALETTE_SHIFTER_0_BASE, 2*i); //offset multiplied by 2 to be on 16-bit boundaries.
	}

	for (i = 0; i < n; i++){
		printf("palette[ 0x%02x ]: 0x%04x\n", 2*i, current_palette.colors[i]);
	}
}
